...
<li class="<?php echo $statr ?>-element" id="task1">
    ...
    <div class="row">
        ...
        <div class="col-lg-6">
            <br>
            <?php if ($do[$i]->berat != '0') { ?>
            <a href="<?php echo base_url('User/pindah3/'.$do[$i]->idProduk.'/'.$do[$i]->kuantitas.'/'.$do[$i]->idProProd.'/'.$do[$i]->idSPK.'/'.$do[$i]->nomorFaktur)?>" onclick="return confirm('Apakah anda yakin untuk mengakhiri aktivitas produksi nomor faktur <?php echo $do[$i]->nomorFaktur ?> dan memindahkan ke inventory ?')"  class="btn btn-xs btn-primary btn-block">Selesai</a>
            <?php } else { ?>
            <button onclick="return confirm('Berat belum diisi')" disabled class="btn btn-xs btn-primary btn-block">Selesai
            </button>
        <?php } ?>
        </div>
    </div>
